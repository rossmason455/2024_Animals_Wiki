            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-6 ">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <!-- Search Form -->
                        <form action="{{ route('families.index') }}" method="GET" class="relative mb-4">
                            <input type="text" id="search" name="search" placeholder="Search families..."
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 ">
                            @foreach($families as $family)
                                <a href="{{ route('families.show', $family) }}">
                                    <x-family-card :family_name="$family->family_name"
                                        :characteristics="$family->characterisitcs"
                                        :evolutionary_origin="$family->evolutionary_origin" :image="$family->image_url" />
                                </a>
                            @endforeach
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#search').on('input', function () {
            ...
            if (query.length > 2) {
                // Perform an AJAX GET request to the "families.autocomplete" route
                $.ajax({
                    url: "{{ route('families.autocomplete') }}", // URL of the autocomplete route
